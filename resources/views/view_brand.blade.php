            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach ($brand as $brands)
                    <div class="bg-white p-6 rounded-md shadow-md mb-4">
                        <img src="{{ asset('storage/'.$brands->logo)}}" alt=""  class="w-auto h-40 object-contain rounded-lg mx-auto mt-2 mb-8">
                        <h2 class="text-2xl font-bold mb-2 text-center">
                            <a href="/brand/{{ $brands['id'] }}" class="text-blue-500 hover:underline">{!! nl2br($brands['brand_name']) !!}</a>
                        </h2>
                        <div class="flex justify-between mt-2">
                            <a href="{{route('adminedit_brand', $brands)}}" class="text-white opacity-10 hover:underline">Edit</a>
...
<form action="{{ route('admincreate_brand')}}" method="GET" class="mb-4">
    <div class="flex items-center justify-center mt-8 mb-4">
        <button class="bg-blue-100 text-blue-100 px-4 py-2 rounded-md" href="{{ route('admincreate_brand')}}">Create</button>
    </div>
</form>
